...
bool has_tmp_suffix(const char* name) {
...
bool CacheStore::read(const char* key, uint16_t schema_version, void* data, size_t size) {
    ...
    if (!header_ok || header.magic != kMagic || header.schema_version != schema_version ||
        ...
        std::fclose(f);
        // Self-heal: a mismatched/corrupt cache file should not keep
        // surviving forever and generating the same warning on every boot.
        std::remove(final_path);
        ESP_LOGI(kTag, "read('%s'): missing/corrupt/schema mismatch - dropped old cache", key);
        return false;
    }
...
void CacheStore::cleanup_interrupted_writes() {
    DIR* dir = opendir(kBasePath);
    if (!dir) return;

    while (dirent* entry = readdir(dir)) {
        if (!has_tmp_suffix(entry->d_name)) continue;

        char tmp_path[kMaxPath];
        std::snprintf(tmp_path, sizeof(tmp_path), "%s/%s", kBasePath, entry->d_name);
        std::remove(tmp_path);
        ESP_LOGI(kTag, "removed interrupted temp cache '%s'", entry->d_name);
    }
    closedir(dir);
}
