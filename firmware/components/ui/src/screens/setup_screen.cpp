...
void render_wifi_list() {
    ...
    for (std::size_t index = 0; index < g_last_state.setup.wifi_networks.size(); ++index) {
        ...
        lv_obj_set_style_bg_color(button,
                                  index == g_selected_wifi_index ? ui_theme::color_panel()
        ...
        std::snprintf(row_text, sizeof(row_text), "%s  •  %s  •  %d dBm",
...
lv_obj_t* build_setup_screen(lv_obj_t* parent) {
    ...
    lv_obj_set_scroll_dir(ctx.root, LV_DIR_VER);

    ctx.title = lv_label_create(ctx.root);
    ...
    lv_obj_set_height(ctx.wifi_panel, LV_SIZE_CONTENT);
    lv_obj_set_style_bg_color(ctx.wifi_panel, ui_theme::color_panel(), 0);
    lv_obj_set_style_border_color(ctx.wifi_panel, ui_theme::color_border(), 0);
    lv_obj_set_style_pad_all(ctx.wifi_panel, 12, 0);
    lv_obj_set_flex_flow(ctx.wifi_panel, LV_FLEX_FLOW_COLUMN);
    lv_obj_set_style_pad_row(ctx.wifi_panel, 8, 0);

    ctx.wifi_selected = lv_label_create(ctx.root);
    lv_obj_set_width(ctx.wifi_selected, LV_PCT(100));
    lv_label_set_long_mode(ctx.wifi_selected, LV_LABEL_LONG_WRAP);
    lv_obj_set_style_text_font(ctx.wifi_selected, ui_theme::font_small(), 0);
    lv_obj_set_style_text_color(ctx.wifi_selected, ui_theme::color_text(), 0);

    lv_obj_move_to_index(ctx.wifi_selected, -1);
    lv_obj_set_parent(ctx.wifi_selected, ctx.wifi_panel);

    ctx.wifi_status = lv_label_create(ctx.wifi_panel);
    ...
    ctx.wifi_password_label = lv_label_create(ctx.wifi_panel);
    ...
    ctx.wifi_password = lv_textarea_create(ctx.wifi_panel);
    ...
    ctx.wifi_list = lv_obj_create(ctx.wifi_panel);
    lv_obj_set_width(ctx.wifi_list, LV_PCT(100));
    lv_obj_set_height(ctx.wifi_list, 180);
    ...
    lv_obj_set_height(ctx.timezone_panel, LV_SIZE_CONTENT);
    lv_obj_set_style_bg_color(ctx.timezone_panel, ui_theme::color_panel(), 0);
    lv_obj_set_style_border_color(ctx.timezone_panel, ui_theme::color_border(), 0);
    lv_obj_set_style_pad_all(ctx.timezone_panel, 12, 0);
    lv_obj_set_flex_flow(ctx.timezone_panel, LV_FLEX_FLOW_COLUMN);
    lv_obj_set_style_pad_row(ctx.timezone_panel, 8, 0);
    ...
    lv_obj_set_height(ctx.confirmation_panel, LV_SIZE_CONTENT);
    lv_obj_set_style_bg_color(ctx.confirmation_panel, ui_theme::color_panel(), 0);
    lv_obj_set_style_border_color(ctx.confirmation_panel, ui_theme::color_border(), 0);
    lv_obj_set_style_pad_all(ctx.confirmation_panel, 12, 0);
    lv_obj_set_flex_flow(ctx.confirmation_panel, LV_FLEX_FLOW_COLUMN);
    lv_obj_set_style_pad_row(ctx.confirmation_panel, 8, 0);
...
void update_setup_screen(const AppState& state) {
    ...
    set_text_if_changed(g_setup_screen.timezone_value, timezone.label);
    set_text_if_changed(g_setup_screen.format_value,
                        g_selected_use_24h ? strings_ptbr::kSetupFormat24h
                                           : strings_ptbr::kSetupFormat12h);

    char confirmation_wifi[128];
    char confirmation_timezone[128];
    ...
    std::snprintf(confirmation_timezone, sizeof(confirmation_timezone), "%s: %s",
                  strings_ptbr::kSetupConfirmationTimezone, timezone.label);
