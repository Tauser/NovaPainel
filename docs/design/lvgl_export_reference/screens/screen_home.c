 ...
 * SCREEN: Home (Início)
 * Layout — 2 columns:
 *   Left  362 px : Clock hero + Weather widget
 *   Right flex-1 : Agenda list · Quick scenes · Player + Market
 * ================================================================ */
#include "../novapanel.h"
#include <stdio.h>
...
void np_screen_home(lv_obj_t *parent)
{
     ...
     * LEFT COLUMN
     * ══════════════════════════════════════════ */
    lv_obj_t *left = np_card(parent);           /* created in parent first */
    lv_obj_set_parent(left, scr);               /* then re-parented to scr  */
    ...
    lv_obj_t *hhmm = lv_label_create(clk_row);
    lv_label_set_text(hhmm, "09:42");
    lv_obj_set_style_text_font(hhmm,  NP_F_HERO, 0);
    lv_obj_set_style_text_color(hhmm, NP_C_TEXT, 0);

    lv_obj_t *secs = lv_label_create(clk_row);
    lv_label_set_text(secs, "30");
    lv_obj_set_style_text_font(secs,  NP_F_3XL, 0);
    lv_obj_set_style_text_color(secs, lv_color_hex(0x3A4252), 0);
    ...
    lv_obj_t *tmp = np_label(wr, "24°", NP_F_4XL, NP_C_TEXT);
    (void)tmp;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        lv_obj_t *vl = np_label(sc, stats[i].v, NP_F_MD, lv_color_hex(stats[i].c));
    ...
    lv_obj_t *ag_ic = np_label(agh, LV_SYMBOL_LIST, NP_F_SM, NP_C_ACCENT);
    (void)ag_ic;
    lv_obj_t *ag_t = np_label(agh, "Agenda de hoje", NP_F_SM, NP_C_TEXT);
    lv_obj_set_flex_grow(ag_t, 1);
    ...
    for (int i = 0; i < 2; i++) {
        lv_obj_t *sb = lv_btn_create(sc_row);
    ...
    lv_obj_center(cov_ic);

    lv_obj_t *si = np_col(ptop);
    lv_obj_set_flex_grow(si, 1);
    ...
    lv_obj_t *play = lv_btn_create(ctrl);
    ...
    lv_obj_set_flex_align(mk,
        LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START);
    lv_obj_set_style_pad_row(mk, 0, 0);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        lv_obj_t *an = np_label(ar, mkt[i].n, NP_F_SM, lv_color_hex(0x5A6478));
        lv_obj_set_flex_grow(an, 1);
