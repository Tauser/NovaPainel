/* ================================================================
 * SCREEN: Mercado (Market / Candlestick)
 * Layout — 2 columns:
 *   Left  flex-1 : BTC chart panel (canvas placeholder) + OHLC
 *   Right 256px  : USD/BRL · Ibovespa · Meta / Favorites
 * ================================================================ */
#include "../novapanel.h"
#include <stdio.h>

/* Candlestick data (simplified for canvas draw) */
...
static void draw_candles(lv_event_t *e)
{
    ...
    lv_area_t obj_area;
    lv_obj_get_coords(canvas, &obj_area);

    int32_t w = lv_area_get_width(&obj_area);
    int32_t h = lv_area_get_height(&obj_area);

    /* price range for this batch */
    int32_t lo = 96000, hi = 105000;
    int32_t range = hi - lo;

    int32_t cw   = w / (int32_t)CANDLE_N;  /* candle slot width */
    int32_t gap  = cw / 4;
    int32_t bw   = cw - gap * 2;           /* body width */
    if (bw < 2) bw = 2;
    ...
    for (int i = 0; i < (int)CANDLE_N; i++) {
        ...
        lv_point_t wp1 = { x0 + bw/2, yH };
        lv_point_t wp2 = { x0 + bw/2, yL };
        lv_draw_line(draw_ctx, &wick, &wp1, &wp2);
        ...
        if (ba.y1 == ba.y2) ba.y2 = ba.y1 + 1; /* minimum 1px body */
        lv_draw_rect(draw_ctx, &body, &ba);
    }
}
...
void np_screen_mercado(lv_obj_t *parent)
{
    ...
    lv_obj_t *bh = np_col(lh);
    lv_obj_set_size(bh, LV_SIZE_CONTENT, LV_SIZE_CONTENT);
    np_label(bh, "Bitcoin · BTC/USD", NP_F_SM, NP_C_TEXT);

    lv_obj_t *sp = lv_obj_create(lh);
    ...
    lv_obj_set_style_bg_color(per, NP_C_CARD2, 0);
    ...
    for (int i = 0; i < 4; i++) {
        ...
        lv_obj_set_style_bg_color(pb, i == 1 ? lv_color_hex(0x252A3C) : NP_C_CARD2, 0);
    ...
    lv_obj_t *sp2 = lv_obj_create(pr);
    np_obj_clear_style(sp2);
    ...
    lv_obj_set_style_pad_row(usd, 0, 0);
