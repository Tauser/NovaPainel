 ...
 * SCREEN: Agenda
 * Layout — 2 columns:
 *   Left  flex-1.35 : Next-event banner + scrollable event list
 *   Right flex-1    : Summary grid + Tomorrow
 * ================================================================ */
#include "../novapanel.h"
#include <stdio.h>
...
void np_screen_agenda(lv_obj_t *parent)
{
    ...
    lv_obj_set_style_border_left_width(nxt, 3, 0);
    lv_obj_set_style_border_left_color(nxt, NP_C_ACCENT, 0);
    lv_obj_set_style_border_left_opa(nxt,   LV_OPA_COVER, 0);
    ...
    for (int i = 0; i < 4; i++) {
        ...
        /* delete icon */
        lv_obj_t *del = np_label(row, LV_SYMBOL_CLOSE, NP_F_XS, NP_C_TEXT_MUTED);
        lv_obj_set_style_opa(del, LV_OPA_40, 0);
    }
    ...
    static lv_coord_t cols[] = { lv_pct(50), lv_pct(50), LV_GRID_TEMPLATE_LAST };
    ...
    for (int i = 0; i < 4; i++) {
        ...
        lv_obj_set_style_bg_color(cell, NP_C_CARD2, 0);
    ...
    lv_obj_set_style_border_dash_width(empty, 4, 0);
