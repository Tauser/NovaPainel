...
/* nav rail entries (only the main 7 get rail buttons) */
static const struct { const char *sym; const char *label; np_screen_t scr; }
nav_items[] = {
...
static void np_build_rail(void)
{
    ...
    for (int i = 0; i < (int)NAV_N; i++) {
        lv_obj_t *btn = lv_btn_create(np_rail);
...
/* ================================================================
 * Topbar  (content area width × 54 px)
 * ================================================================ */
static void np_build_topbar(void)
{
    ...
    lv_obj_set_size(tb, 0, NP_TOPBAR_H);
...
{
    /* root */
    np_root = lv_obj_create(lv_scr_act());
...
/* ================================================================
 * Tick — call every 1 s from a timer task
 * ================================================================ */
void np_tick(void)
{
    ...
    /* update topbar subtitle with HH:MM */
    ...
    if (np_state.timer_running && np_state.timer_sec > 0) {
        ...
        /* caller should redraw timer arc after np_tick() */
    }
}
