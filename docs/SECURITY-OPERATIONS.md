...
Guia operacional da trilha H3.

> Leitura correta no baseline atual:
>
> Este documento preserva a politica de seguranca e release definida para o
> projeto, mas parte dela ainda e patrimonio tecnico/documental da trilha H e
> pode depender de reentrega tecnica no `firmware/` novo antes de ser tratada
> como fluxo operacional ativo do tree atual.

Consolida o uso de `firmware/sdkconfig.prod`, `firmware/partitions.csv` e as
ADRs `0011`/`0030` para evitar erro humano em build, provisionamento e
manutencao de unidades de producao.
...
- politica de seguranca: **valida e canonica**
- base documental da trilha H3: **consolidada**
- fluxo de release PROD do `firmware/` novo: **depende da Fase 7 do roadmap**

Isso evita uma leitura errada de que todo o fluxo abaixo ja esta operacional no
baseline atual do reboot.
...
- a politica aqui continua sendo a referencia para a **Fase 7 - Hardening de
  release e seguranca PROD**
- antes disso, o `firmware/` novo pode ainda nao ter reentregado todo o fluxo
  funcional necessario para release de produto
